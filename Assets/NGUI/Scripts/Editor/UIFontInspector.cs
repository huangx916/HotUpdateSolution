...
public class UIFontInspector : Editor
{
	...
	public override void OnInspectorGUI ()
	{
		...
		{
			int size = EditorGUILayout.IntField("Font Size", mFont.bmFont.charSize);
			if (size != mFont.bmFont.charSize)
			{
				NGUIEditorTools.RegisterUndo("Font change", mFont);
				mFont.bmFont.charSize = size;
			}
	...
	public override void OnPreviewGUI (Rect rect, GUIStyle background)
	{
		...
		if (mView != View.Nothing && tex != null)
		{
			...
			UIAtlas atlas = mFont.atlas;
			if (mView == View.Font && mFont.atlas != null && mFont.sprite != null)
			{
				NGUIEditorTools.DrawSprite(tex, rect, mFont.sprite, Color.white, m, atlas.width, atlas.height);
			}
			else
			{
