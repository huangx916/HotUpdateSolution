	...
	protected override bool ShouldDrawProperties ()
	{
		...
		if (mPanel.clipping != UIDrawCall.Clipping.None)
		{
			...
			if (mPanel.clipping == UIDrawCall.Clipping.SoftClip)
			{
				GUILayout.BeginHorizontal();
				GUILayout.Space(80f);
				Vector2 corner = EditorGUILayout.Vector2Field("Corner", mPanel.clipCorner, GUILayout.MinWidth(20f));
				GUILayout.EndHorizontal();

				if (corner.x < 0f) corner.x = 0f;
				if (corner.y < 0f) corner.y = 0f;
				...
				if (corner.x > 0 && corner.y > 0)
				{
					GUILayout.BeginHorizontal();
					GUILayout.Space(80f);
					float cornerBorder = EditorGUILayout.FloatField("Border Rate", mPanel.cornerBorder, GUILayout.MinWidth(20f));
					GUILayout.EndHorizontal();

					GUILayout.BeginHorizontal();
					GUILayout.Space(80f);
					float cornerSoft = EditorGUILayout.FloatField("Soft Rate", mPanel.cornerSoft, GUILayout.MinWidth(20f));
					GUILayout.EndHorizontal();

					if (cornerBorder < 0f) cornerBorder = 0f;
					if (cornerBorder > 1) cornerBorder = 1;

					if (cornerSoft < 0f) cornerSoft = 0f;
					if (cornerSoft  > 0.5F) cornerSoft = 0.5F;

					if (mPanel.clipCorner != corner || mPanel.cornerSoft != cornerSoft || mPanel.cornerBorder != cornerBorder)
					{
						NGUIEditorTools.RegisterUndo("Clipping Change", mPanel);
						mPanel.clipCorner = corner;
						mPanel.cornerBorder = cornerBorder;
						mPanel.cornerSoft = cornerSoft;
						EditorUtility.SetDirty(mPanel);
					}
				}
				else
				{
					GUILayout.BeginHorizontal();
					GUILayout.Space(80f);
					soft = EditorGUILayout.Vector2Field("Softness", mPanel.clipSoftness, GUILayout.MinWidth(20f));
					GUILayout.EndHorizontal();
					
					if (soft.x < 0f) soft.x = 0f;
					if (soft.y < 0f) soft.y = 0f;

					if (mPanel.clipCorner != corner || mPanel.clipSoftness != soft)
					{
						NGUIEditorTools.RegisterUndo("Clipping Change", mPanel);
						mPanel.clipCorner = corner;
