...
static public class NGUIEditorTools
{
	...
	{
		if (!tex) return;
		DrawSprite(tex, rect, sprite, color, tex.width, tex.height);
	}
	...
	{
		DrawSprite(tex, rect, sprite, color, null, texWidth, texHeight);
	}
	...
	{
		if (!tex) return;
		DrawSprite(tex, drawRect, sprite, color, mat, tex.width, tex.height);
	}
		...
		int texWidth, int texHeight)
	{
		...
		DrawSprite(tex, drawRect, color, mat, sprite.x, sprite.y, sprite.width, sprite.height,
			sprite.borderLeft, sprite.borderBottom, sprite.borderRight, sprite.borderTop, texWidth, texHeight);
	}
	...
	{
		if (!tex) return;
		DrawSprite(tex, drawRect, color, textureRect, border, tex.width, tex.height);
	}
		...
		int texWidth, int texHeight)
	{
		...
		if (mat == null)
		{
			...
			uv = NGUIMath.ConvertToTexCoords(uv, texWidth, texHeight);
