...
public class UIAtlasInspector : Editor
{
		...
		if (mAtlas.spriteMaterial != null)
		{
			...
			if (sprite != null)
			{
				...
				if (tex != null)
				{
					...
					if (GUILayout.Button("Pointer Clone", GUILayout.Width(width * 0.5F - 22)))
					{
						List<string> names = new List<string>();
						foreach (UISpriteData spriteItem in mAtlas.spriteList)
						{
							names.Add(spriteItem.name);
						}
						string name = sprite.name + "(Copy)";
						int i = 0;
						while (names.Contains(name))
						{
							i++;
							name = sprite.name + "(Copy)" + i;
						}
						UISpriteData spriteTemp = new UISpriteData();
						spriteTemp.CopyFrom(sprite);
						spriteTemp.name = name;
						Debug.LogError(spriteTemp.name);
						mAtlas.spriteList.Add(spriteTemp);
					}

					if (GUILayout.Button("Pointer Delete"))
					{
	...
	public override void OnPreviewGUI (Rect rect, GUIStyle background)
	{
		...
		if (tex != null) NGUIEditorTools.DrawSprite(tex, rect, sprite, Color.white, mAtlas.width, mAtlas.height);
	}
