...
public class UILabelInspector : UIWidgetInspector
{
	...
	protected override bool ShouldDrawProperties ()
	{
		...
		EditorGUI.BeginDisabledGroup(!isValid);
		{
			...
			{
				GUILayout.BeginHorizontal();
				SerializedProperty colorSp = NGUIEditorTools.DrawProperty("", serializedObject, "mEffectColor", GUILayout.MinWidth(10f));
				GUI.SetNextControlName("Encoded Color");
				mEncodedEffectColor = EditorGUILayout.TextField("", mEncodedEffectColor, GUILayout.Width(50f));
				GUILayout.EndHorizontal();
				if (colorSp != null)
				{
					if (GUI.GetNameOfFocusedControl() == "Encoded Color")
					{
						if (Event.current.keyCode == KeyCode.Return && Event.current.type == EventType.Used)
						{
							...
							for (int index = mEncodedEffectColor.Length; index < 6; ++index)
							{
								builder.Append('0');
							}
							mEncodedEffectColor += builder;
							colorSp.colorValue = NGUIText.ParseColor24(mEncodedEffectColor, 0);
						}
					}
					else
					{
						mEncodedEffectColor = NGUIText.EncodeColor24(colorSp.colorValue);
					}
				}
				GUILayout.EndHorizontal();
