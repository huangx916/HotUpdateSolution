...
#region Create
	...
	[MenuItem("NGUI/Create/Texture Ex &#t", false, 6)]
	static public void AddTextureEx ()
	{
		GameObject go = NGUIEditorTools.SelectedRoot(true);

		if (go != null)
		{
			Selection.activeGameObject = NGUISettings.AddTextureEx(go).gameObject;
		}
		else Debug.Log("You must select a game object first.");
	}
	...
	[MenuItem("NGUI/Create/Effect &#e", false, 6)]
	static public void AddEffect()
	{
		...
		if (go != null)
		{
			Selection.activeGameObject = NGUISettings.AddEffect(go).gameObject;
		}
		else
