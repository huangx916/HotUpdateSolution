...
public class UIPrefabTool : EditorWindow
{
	...
	static string[] sTabNames = { "NGUI", "Btn", "Panel", "Other", "4", "5" };
	...
	GUIContent mContent;
	GUIStyle mStyle;
	bool[] mTabLoaded = new bool[sTabNames.Length];
	...
	public void Reset ()
	{
		...
		else
		{
			...
			foreach (string s in allAssets)
			{
				if (s.EndsWith(".prefab") && s.Contains(sTabNames[mTab] + "/Control -"))
					filtered.Add(s);
			}
	...
	void Load ()
	{
		...
		if (mTab == 0 && !string.IsNullOrEmpty(data))
		{
			mTabLoaded[mTab] = true;
		}
		if (!mTabLoaded[mTab])
		{
			mTabLoaded[mTab] = true;
	...
	void OnGUI ()
	{
		...
		if (GUILayout.Toggle(newTab == 1, sTabNames[1], "ButtonLeft")) newTab = 1;
		if (GUILayout.Toggle(newTab == 2, sTabNames[2], "ButtonMid")) newTab = 2;
		if (GUILayout.Toggle(newTab == 3, sTabNames[3], "ButtonMid")) newTab = 3;
		if (GUILayout.Toggle(newTab == 4, sTabNames[4], "ButtonMid")) newTab = 4;
		if (GUILayout.Toggle(newTab == 5, sTabNames[5], "ButtonRight")) newTab = 5;
		if (GUILayout.Toggle(newTab == 0, sTabNames[0], "Button")) newTab = 0;
