...
public class UIAtlasMaker : EditorWindow
{
	...
	List<Texture> GetSelectedTextures ()
	{
		...
		if (Selection.objects != null && Selection.objects.Length > 0)
		{
			...
			foreach (Object o in objects)
			{
				...
				if (NGUISettings.atlas == null || (NGUISettings.atlas.texture != tex && NGUISettings.atlas.alphaTexture != tex)) textures.Add(tex);
			}
		}
		return textures;
	}
	...
	Dictionary<string, int> GetSpriteList (List<Texture> textures)
	{
		...
		if (textures.Count > 0)
		{
			...
			foreach (string tex in texNames)
			{
				...
				{
					Debug.LogError("Texture name [" + tex + "] is already exist!");
				}
				else
				{
	...
	static public SpriteEntry DuplicateSprite (UIAtlas atlas, string spriteName)
	{
		...
		Texture2D alphaTex = NGUIEditorTools.ImportTexture(atlas.alphaTexture, true, true, false);
		SpriteEntry se = ExtractSprite(sd, tex, alphaTex);
	...
	static SpriteEntry ExtractSprite (UISpriteData es, Texture2D tex, Texture2D alphaTex)
	{
		return (tex != null) ? ExtractSprite(es, tex.GetPixels32(), alphaTex != null ? alphaTex.GetPixels32() : null, tex.width, tex.height) : null;
	}
		...
		if (tex != null)
		{
			...
			foreach (UISpriteData es in sprites)
			{
				...
				if (!found)
				{
					...
					if (alphaPixels == null && atlas.alphaTexture != null)
					{
						alphaPixels = NGUIEditorTools.ImportTexture(atlas.alphaTexture, true, true, false).GetPixels32();
					}
					SpriteEntry sprite = ExtractSprite(es, pixels, alphaPixels, width, height);
		...
		if (PackTextures(tex, sprites))
		{
			...
			if (newTexture)
			{
				...
				else
				{
					...
					if (alphaTex != null) atlas.spriteMaterial.SetTexture("_AlphaTex", alphaTex);
				}
	...
	static public Texture2D AlphaSplit(ref Texture2D tex)
	{
		int width = tex.width;
		int height = tex.height;
		if (width > 0 && height > 0)
		{
			...
			Color32[] rgbPixels = new Color32[width * height];
			Color32[] alphaPixels = new Color32[width * height];
			...
			for (int y = 0; y < height; ++y)
			{
				for (int x = 0; x < width; ++x)
				{
					...
					alphaPixels[index] = new Color32(color.a, color.a, color.a, 1);
				}
			}
	...
	static public Texture2D AlphaMerge(Texture2D rgbTex, Texture2D alphaTex)
	{
	...
	static public void UpdateAtlas (UIAtlas atlas, List<SpriteEntry> sprites)
	{
		if (sprites.Count > 0)
		{
			if (!NGUISettings.atlasAlphaSplit && atlas.spriteMaterial.HasProperty("_AlphaTex"))
				atlas.spriteMaterial.SetTexture("_AlphaTex", null);
