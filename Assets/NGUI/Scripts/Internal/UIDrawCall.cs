...
public class UIDrawCall : MonoBehaviour
{
	...
	[System.NonSerialized] Texture		mAlphaTex;      // Alpha texture used by the material
	...
	public Texture alphaTexture
	{
		...
		set
		{
			mAlphaTex = value;
			if (mBlock == null) mBlock = new MaterialPropertyBlock();
			mBlock.SetTexture("_AlphaTex", value ?? Texture2D.whiteTexture);
		}
	}
	...
	void CreateMaterial ()
	{
		...
		{
			mTextureClip = true;
			shader = ShaderCache.Find("Hidden/" + shaderName + textureClip);
		}
		else if (mClipCount != 0)
		{
			shader = ShaderCache.Find("Hidden/" + shaderName + " " + mClipCount);
			if (shader == null) shader = ShaderCache.Find(shaderName + " " + mClipCount);

			// Legacy functionality
			if (shader == null && mClipCount == 1)
			{
				mLegacyShader = true;
				shader = ShaderCache.Find(shaderName + soft);
			}

			if (shader == null) shader = ShaderCache.Find(shaderName);
		}
		else shader = ShaderCache.Find(shaderName);

		// Always fallback to the default shader
		if (shader == null) shader = ShaderCache.Find("Unlit/Transparent Colored");
	...
	/// <summary>
	/// Clear the draw call's geometry.
	/// </summary>

	public void UpdateBlankGeometry()
	{
		this.widgetCount = 1;
	...
	void SetClipping (int index, Vector4 cr, Vector2 soft, Vector2 cor, float corSoft, float corBorder, float angle)
	{
		angle *= -Mathf.Deg2Rad;
		...
		if (index < ClipRange.Length)
		{
			...
			mDynamicMat.SetVector("_ClipConer", new Vector4(cor.x / cr.z, cor.y / cr.w, corSoft == 0 ? 1000 : 1 / corSoft, corBorder));
		}
	}
	...
	static public UIDrawCall Create (UIPanel panel, Material mat, Texture tex, Texture alphaTex, Shader shader)
	{
#if UNITY_EDITOR
		string name = null;
		...
		return Create(name, panel, mat, tex, alphaTex, shader);
#else
		return Create(null, panel, mat, tex, alphaTex, shader);
#endif
	}
	...
	static UIDrawCall Create (string name, UIPanel pan, Material mat, Texture tex, Texture alphaTex, Shader shader)
	{
