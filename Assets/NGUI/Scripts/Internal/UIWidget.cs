...
public class UIWidget : UIRect
{
	...
	[System.NonSerialized] public UIDrawCall mDrawCall;
	public virtual UIDrawCall drawCall
	{
		get
		{
			return mDrawCall;
	...
	public virtual bool forceDraw { get { return false; } }
	...
	static public int PanelCompareFunc (UIWidget left, UIWidget right)
	{
		...
		// if not both material is null, sort by material;
		...
		if (leftMat != null) return 0;

		// if both material is null, but not both support only texture, support is behind;
		bool leftSupportSampleTexture = left.isSupportOnlyTexture;
		bool rightSupportSampleTexture = right.isSupportOnlyTexture;
		if (leftSupportSampleTexture != rightSupportSampleTexture)
		{
			if (leftSupportSampleTexture) return 1;
			return -1;
		}
		if (!leftSupportSampleTexture) return 0;

		// if both support only texture, but not both main texture is null, sort by main texture;
		Texture leftMainTexture = left.mainTexture; 
		Texture rightMainTexture = right.mainTexture;
		if (leftMainTexture != rightMainTexture)
		{
			if (leftMainTexture == null) return -1;
			if (rightMainTexture == null) return 1;
			return (leftMainTexture.GetInstanceID() < rightMainTexture.GetInstanceID()) ? -1 : 1;
		}
		if (leftMainTexture == null) return 0;

		// if both main texture is null, but not both alpha texture is null, sort by alpha texture;
		Texture leftAlphaTexture = left.alphaTexture;
		Texture rightAlphaTexture = right.alphaTexture;
		if (leftAlphaTexture != rightAlphaTexture)
		{
			if (leftAlphaTexture == null) return -1;
			if (rightAlphaTexture == null) return 1;
			return (leftAlphaTexture.GetInstanceID() < rightAlphaTexture.GetInstanceID()) ? -1 : 1;
		}
		return 0;
	}
	...
	/// </summary>
	protected virtual bool isSupportOnlyTexture
	{
		get
		{
			return false;
	...
	[System.NonSerialized] Texture mOldAlphaTex;
	...
	protected override void OnValidate()
	{
		if (NGUITools.GetActive(this))
		{
			...
			if (mOldTex != mainTexture || mOldAlphaTex != alphaTexture || mOldShader != shader)
			{
				mOldTex = mainTexture;
				mOldAlphaTex = alphaTexture;
				mOldShader = shader;
			}
	...
	virtual public void MakePixelPerfect ()
	{
		...
		if (this.GetType() == typeof(UIWidget))
		{
			Vector2 extents = new Vector2(2, 2);
			Vector2 offset = pivotOffset;
			int[,] extentsLimit = { { offset.x > 0 ? 1 : 0, offset.x < 1 ? 1 : 0}, { offset.y > 0 ? 1 : 0, offset.y < 1 ? 1 : 0 } };
			...
			foreach (Transform child in transform)
			{
				...
				{
					Bounds childBounds = NGUIMath.CalculateRelativeWidgetBounds(myTrans, child);
					float minX = childBounds.min.x * extentsLimit[0, 0];
					minX = minX > 0 ? 0 : -minX;

					float maxX = childBounds.max.x * extentsLimit[0, 1];
					if (maxX < 0)
						maxX = 0;

					float minY = childBounds.min.y * extentsLimit[1, 0];
					minY = minY > 0 ? 0 : -minY;

					float maxY = childBounds.max.y * extentsLimit[1, 1];
					if (maxY < 0)
						maxY = 0;

					extents.x = Mathf.Max(extents.x, Mathf.Max(minX, maxX));
					extents.y = Mathf.Max(extents.y, Mathf.Max(minY, maxY));
				}
			}
			width = Mathf.RoundToInt(offset.x == 0.5F ? extents.x + extents.x : extents.x);
			height = Mathf.RoundToInt(offset.y == 0.5F ? extents.y + extents.y : extents.y);
		}
	}
	...
	/// <summary>
	/// Dimensions of the sprite's flipped border, if any.
	/// </summary>
