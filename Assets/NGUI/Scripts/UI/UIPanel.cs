...
public class UIPanel : UIRect
{
	...
	[HideInInspector][SerializeField] Vector2 mClipCorner = new Vector2(0, 0);
	[HideInInspector][SerializeField] float mCornerSoft = 0;
	[HideInInspector][SerializeField] float mCornerBorder = 0;
		...
		for (int i = 0; i < widgets.Count; ++i)
		{
			...
			if (w.isVisible)
			{
				if (w.forceDraw)
				{
					...
					dc = UIDrawCall.Create(this, null, null, null, null);
				...
				else
				{
					if (w.hasVertices)
					{
						...
						Texture aTx = w.alphaTexture;
						Shader sd = w.shader;

						if (mat != mt || tex != tx || aTex != aTx || sdr != sd)
						{
						...
						if (mat != null || sdr != null || tex != null)
						{
							if (dc == null)
							{
								dc = UIDrawCall.Create(this, mat, tex, aTex, sdr);
								dc.depthStart = w.depth;
								dc.depthEnd = dc.depthStart;
								dc.panel = this;
								dc.onCreateDrawCall = onCreateDrawCall;
	...
	public UIDrawCall FindDrawCall (UIWidget w)
	{
		...
		for (int i = 0; i < drawCalls.Count; ++i)
		{
			...
			{
				if (dc.baseMaterial == mat && dc.shader == shader && dc.mainTexture == tex && dc.alphaTexture == aTex)
				{
					if (w.isVisible)
					{
						w.drawCall = dc;
	...
	void OnDrawGizmos ()
	{
		...
		if (detailedView)
		{
			if (detailedClipped)
			{
				...
				if (mClipCorner.x == 0 || mClipCorner.y == 0)
				{
					size.x -= mClipSoftness.x * 2f;// *scale;
					size.y -= mClipSoftness.y * 2f;// *scale;
				}
				else if (mCornerBorder == 0)
				{
					size.x -= mClipCorner.x * mCornerSoft * 2f;// *scale;
					size.y -= mClipCorner.y * mCornerSoft * 2f;// *scale;
				}
				else
				{
					float thickness = 1 - mCornerBorder;
					size.x -= mClipCorner.x * thickness * 2f;// *scale;
					size.y -= mClipCorner.y * thickness * 2f;// *scale;
				}
				Gizmos.DrawWireCube(pos, size);
			}
		}
