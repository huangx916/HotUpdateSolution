...
public class UI2DSprite : UIBasicSprite
{
	...
	public UnityEngine.Sprite sprite2D
	{
		...
		set
		{
			if (mSprite as object != value as object)
			{
				RemoveFromPanel();
				mSprite = value;
				nextSprite = null;
	...
	public UnityEngine.Sprite alphaSprite2D
	{
		...
		set
		{
			if (mAlphaSprite as object != value as object)
			{
				RemoveFromPanel();
				mAlphaSprite = value;
				CreatePanel();
	...
	public override Texture alphaTexture
	{
		get
		{
			if (mAlphaSprite != null) return mAlphaSprite.texture;
	...
	public override void MakePixelPerfect ()
	{
		...
		else if (hasBorder)
		{
			if (mType == Type.Tiled)
			{
				...
				int w = Mathf.RoundToInt(pixelSize * wTemp * rect.width / tex.width);
				...
				int h = Mathf.RoundToInt(pixelSize * hTemp * rect.height / tex.height);
			...
			else
			{
				...
				if (mMirror == Flip.Horizontally || mMirror == Flip.Both)
				{
					if (mFlip == Flip.Horizontally || mFlip == Flip.Both)
						wTemp = Mathf.RoundToInt(wTemp + wTemp - border.x - border.x);
					else
						wTemp = Mathf.RoundToInt(wTemp + wTemp - border.z - border.z);
				}
				int w = Mathf.RoundToInt(pixelSize * wTemp * rect.width / tex.width);
				...
				if (mMirror == Flip.Vertically || mMirror == Flip.Both)
				{
					if (mFlip == Flip.Vertically || mFlip == Flip.Both)
						hTemp = Mathf.RoundToInt(hTemp + hTemp - border.y - border.y);
					else
						hTemp = Mathf.RoundToInt(hTemp + hTemp - border.w - border.w);
				}
				int h = Mathf.RoundToInt(pixelSize * hTemp * rect.height / tex.height);
		...
		{
			float wTemp = rect.width;
			if (mMirror == Flip.Horizontally || mMirror == Flip.Both)
				wTemp += wTemp;
			int w = Mathf.RoundToInt(pixelSize * wTemp);

			float hTemp = rect.height;
			if (mMirror == Flip.Vertically || mMirror == Flip.Both)
				hTemp += hTemp;
			int h = Mathf.RoundToInt(pixelSize * hTemp);
