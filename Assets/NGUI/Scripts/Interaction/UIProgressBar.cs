...
public class UIProgressBar : UIWidgetContainer
{
	...
	public virtual void ForceUpdate ()
	{
		...
		if (mFG != null)
		{
			...
			bool simpleOrSliced = sprite && (sprite.type == UIBasicSprite.Type.Simple || sprite.type == UIBasicSprite.Type.Sliced);
			bool simpleOrSlicedFill = simpleOrSliced && sprite.fillDirection != UIBasicSprite.FillDirection.Nothing;
			bool willClip = simpleOrSlicedFill || (sprite && sprite.type == UIBasicSprite.Type.Filled);

			if (isHorizontal)
			{
				if (willClip)
				{
					if (sprite.fillDirection == UIBasicSprite.FillDirection.Nothing ||
					...
					mFG.drawRegion = new Vector4(0f, 0f, 1f, 1f);
				}
				else
				{
